...
import kiosk.bank.v2.Bank.minStorageRent
...
class UpdateRootSpec extends PropSpec with Matchers with ScalaCheckDrivenPropertyChecks with HttpClientTesting {
  ...
  ignore("Bank can make box functional again on main net") {
    Client.usingContext { implicit ctx: BlockchainContext =>
      noException shouldBe thrownBy {
        ...
        val bankBoxId = "00d6c7f55d83f26a6efe89981ec00fa1d3356f81947b1521b20e1c55bc807005"
        ...
        val digest: Array[Byte] = avlProver.digest

        val expectedR4: KioskAvlTree = KioskAvlTree(digest, KL, Some(VL))
        val actualR4: KioskAvlTree = new KioskAvlTree(bankBox.getRegisters.asScala(0).asInstanceOf[ErgoValue[AvlTree]].getValue)
        actualR4.hex shouldBe expectedR4.hex
        ...
        TxUtil.createTx(
          ...
          Array(
            KioskBox(
              Bank.bankAddress,
              minStorageRentProd,
              registers = Array(KioskAvlTree(digest, KL, Some(VL)), bankPubKey, isNotDefunct),
              tokens = Array((bankNFT, 1), (bankTokenId, bankTokenAmount)),
              creationHeight = Some(ctx.getHeight)
            ),
            KioskBox(
              ...
              minStorageRentProd,
              registers = Array(dummyCollByte1, dummyCollByte2),
              tokens = Array((bankTokenId, 1))
            )
          ),
          fee = 1000000L,
          changeAddress,
          Array(bankSecret, fundingSecret),
          Array[DhtData](),
          true
        )
      }
