...
class WithdrawSpec extends PropSpec with Matchers with ScalaCheckDrivenPropertyChecks with HttpClientTesting {
  ...
  ignore("WithdrawProd") {
    Client.usingContext { implicit ctx: BlockchainContext =>
      ...
      val bankBoxId = "8d13f40194ef5f444c017f3c88a424f90639b9976441e35e7b4fadeb9b5a1e11"

      val fundingSecret = "-21120845569549261709579423621450797100274242707397768518154077936446211979699"
      val fundingAddress = new Address(ScalaErgoConverters.getAddressFromString(changeAddress))
      val fundingBox = ctx.getUnspentBoxesFor(fundingAddress, 0, 10).asScala(0)
      val bankBox = ctx.getBoxesById(bankBoxId)(0)
      println("Funding box Id " + fundingBox.getId)

      bankBox.getErgoTree shouldBe bankBox.getErgoTree
      bankBox.getTokens.get(0).getId.toString shouldBe bankNFT
      bankBox.getTokens.get(0).getValue shouldBe 1
      bankBox.getTokens.get(1).getId.toString shouldBe bankTokenId
      bankBox.getTokens.get(1).getValue shouldBe bankTokenAmount

      println("Context 0 " + KioskCollByte(removeProof).serialize.encodeHex)
      println("Context 1 " + KioskCollByte(lookupProof).serialize.encodeHex)
